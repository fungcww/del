...
<Assembly: ApplicationName("DBAccess")> 
<Assembly: ApplicationActivation(ActivationOption.Server)> 
<Assembly: ApplicationAccessControl(True)> 

<Serializable()> _
Public Class DBSException
...
Public Class Database
    ...
    Shared m_connstr As String = ""
    Private lngErrNo As Long
    Private strErr As String

    Public Function ConnStr(ByVal strProj As String, ByVal strConnType As String, Optional ByVal strUserType As String = "") As String Implements ISecurity.ConnStr

        Dim strSQL, strConnStr As String
        Dim sqlconnect As New SqlConnection
        Dim sqlcmd As New SqlCommand

        strSQL = "DECLARE @pa varchar(250) "
        strSQL &= "exec secsp_get_str_" & strProj & " '" & strUserType & "', '" & strConnType & "', @pa out "
        strSQL &= "SELECT @pa"

        sqlconnect.ConnectionString = m_connstr
        sqlconnect.Open()
        ...
        Try
            strConnStr = sqlcmd.ExecuteScalar()
        Catch sqlex As SqlClient.SqlException
            Throw New DBSException(sqlex.ToString)
        Catch ex As Exception
            Throw New DBSException(ex.ToString)
        Finally
            sqlconnect.Close()
        End Try
        ...
        If InStr(UCase(strConnStr), "ERROR") > 0 Then
            Return strConnStr
        End If

        Dim intPos As Integer
        intPos = InStr(strConnStr, "SQL SERVER")
        If intPos > 0 Then
            strConnStr = Replace(strConnStr, "driver={SQL SERVER};", "")
            strConnStr = Replace(strConnStr, "AutoTranslate=No;", "")
        Else
            'strConnStr = _
            '    "Provider=IBMDA400.DataSource.1;Persist Security Info=False;" & _
            '    strConnStr & _
            '    ";Initial Catalog='';Transport Product=Client Access;SSL=DEFAULT;" & _
            '    "Force Translate=1;Default Collection='';Convert Date Time To Char=TRUE;Catalog Library List='';" & _
    ...
    Protected Overrides Sub Construct(ByVal s As String)
        ...
        Try
            din = New StreamReader(File.OpenRead("C:\PublicKey.Net\db.key"))
        ...
        ' get connection string to security database
        If strKey <> Nothing Then
            m_connstr = DecryptString128Bit(s, strKey)
        End If
    ...
    Private Function StripNullCharacters(ByVal vstrStringWithNulls As String) As String
        ...
        Do While intPosition > 0
            ...
            If intPosition > 0 Then
                ...
                strStringWithOutNulls = Microsoft.VisualBasic.Strings.Left(strStringWithOutNulls, intPosition - 1) & _
