    ...
    Private Sub BuildUI()
        ...
        Try
            Using wsCRS As New CRSWS.CRSWS()
                ...
                'If Not wsCRS.GetMarkin(getCompanyCode(g_McuComp), getEnvCode(), strPolicy, dtPOLALT, strErrMsg) Then
                '    MsgBox("System Error." & vbCrLf & strErrMsg, MsgBoxStyle.Critical + MsgBoxStyle.OkOnly, "Error")
                '    Exit Sub
                'End If
                Dim retDs1 = APIServiceBL.CallAPIBusi(getCompanyCode(g_McuComp), "CHECK_MARK_IN", New Dictionary(Of String, String)() From {{"strInPolicy", strPolicy}})
                ...
                If strPolicyCap <> "" AndAlso strPolicyCap.Trim <> strPolicy.Trim Then
                    ...
                    'End If
                    Dim retDs2 = APIServiceBL.CallAPIBusi(getCompanyCode(g_McuComp), "CHECK_MARK_IN", New Dictionary(Of String, String)() From {{"strInPolicy", strPolicyCap}})
                    If Not IsNothing(retDs2) Then
	                    If retDs2.Tables(0).Rows.Count > 0 Then
                            dtPOLALT.Merge(retDs2.Tables(0).Copy)
	                    End If
                    End If
                End If
