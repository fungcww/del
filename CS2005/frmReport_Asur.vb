...
Public Class frmReport_Asur
    ...
    Private objRptLogic As New clsReportLogic_Asur
...
#End Region
    ...
    Private Sub frmReport_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        dtPriv = objCS.GetPrivRS(98, gUPSystem, lngErr, strErr)

        If lngErr <> 0 Then
            MsgBox("Error loading authority records", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Error")
            Exit Sub
        End If
        ...
        If Not dtPriv Is Nothing Then
            If dtPriv.Rows.Count > 0 Then
                For i As Integer = 0 To dtPriv.Rows.Count - 1
                    If Mid(strUPSMenuCtrl, dtPriv.Rows(i).Item("upsmit_seq_no"), 1) = "1" Then
                        strRptList &= CStr(dtPriv.Rows(i).Item("upsmit_seq_no")) & ","
                    End If
                Next
            End If
        End If
        ...
        'Add Post Sales Call follow up letter
        strRptList &= "19, 65, 66, 68, 67, 69, 70,71,72,73,74, 75, 76, 77, 78, 79, 80, 81, 82, 83"
        '001e
        strSQL = "Select * from " & serverPrefix & "csw_report_list " &
            " Where cswrel_grp_no > 1 " &
            " And cswrel_rpt_no in (" & strRptList & ")" &
            " order by cswrel_file_type, cswrel_rpt_no"

        'sqlconnect.ConnectionString = objSec.ConnStr("CSW", "CS2005", "CIW")
        'strCIWConn = "packet size=4096;user id=vantiveowner;data source=hksqldev1;persist security info=True;initial catalog=vantive;password=ownerdev"
        sqlconnect.ConnectionString = strCIWConn
        'sqlconnect.ConnectionString = "packet size=4096;user id=vantiveowner;data source=hksqldev1;persist security info=True;initial catalog=vantive;password=ownerdev"
    ...
    Private Function PrepareData() As Boolean
        ...
        'MsgBox(dr(0).Item("cswrel_file_name").ToString, MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        ...
        If dr.Length > 0 Then
            Select Case dr(0).Item("cswrel_file_type")
                ...
                Case "P", "W"   ' Word / PDF
                    ...
                    If rptNo = "18" Or rptNo = "67" Or rptNo = "79" Then
                        cmdFax.Enabled = False
                        cmdPreview.Enabled = True
                        cmdPrint.Enabled = True
