Public Class uclParSur_Asur
    ...
    Private bIsULinked As Boolean = False
    Private bIsULife As Boolean = False
...
#End Region
    ...
    Public Sub InitInfo()
        ...
        Try
            ...
            If objPOSHeader.TransID <> 0 Then
                ...
                If Me.DBHeader.CompanyID = "LAC" Or Me.DBHeader.CompanyID = "LAH" Then
                    Dim dsRst As New DataSet
                    If objCI.GetProductFeature_ByPolicyNo(Me.PolicyNumber, dsRst, strErr) Then
                        If dsRst.Tables.Count > 0 AndAlso dsRst.Tables(0).Rows.Count > 0 Then
                            Ctrl_POS_Fund_SWOut.IsTransferByUnit = IIf(dsRst.Tables(0).Rows(0).Item("IsLinked"), True, False)
                            bIsULinked = dsRst.Tables(0).Rows(0).Item("IsLinked")
                            bIsULife = dsRst.Tables(0).Rows(0).Item("IsULife")
                        End If
                    End If
                End If
    ...
    Private Sub HandleInitBody()
        Try
            ...
            'oliver 2023-12-8 commented for Switch Over Code from Assurance to Bermuda
            If objCI.GetSurrRecord(Me.PolicyNumber, strProductType, dsParSurHist, dsSurAlloc, strErr, CDate(dsPolicyHead.Tables(0).Rows(0)("Sys_Bus_Date"))) = False Then
                If Trim(strErr) <> "" Then MsgBox(strErr)
                strErr = ""
                Exit Sub
            End If
    ...
    Private Sub updateClaimValue()
        ...
        If Not objCI.GetClaimsParSurAmt(Me.PolicyNumber, dsSurAlloc, dsReceClaim, strErr, tRefDate, tWDMethod) Then   'LeoG1P1
            ...
            If Utility.Utility.isAssurance(objDBHeader.CompanyID) Then
                If UCase(strErr).Contains("H384") Then
                    MsgBox(String.Format("The account balance left after withdrawal must be greater than {0}{1}", dsPolicyHead.Tables(0).Rows(0)("Curr").ToString.Trim, dsPFundDetails.Tables(1).Rows(0)("MinRemainingAmt")))
                ElseIf UCase(strErr).Contains("ZA04") Then
                    MsgBox(String.Format("The total surrender amount must be greater than {0}{1}", dsPolicyHead.Tables(0).Rows(0)("Curr").ToString.Trim, dsPFundDetails.Tables(1).Rows(0)("MinWithdrawalAmt")))
                Else
                    MsgBox(strErr)
                End If
            Else
        ...
        Else
            If dsReceClaim.Tables.Count > 0 AndAlso dsReceClaim.Tables(0).Rows.Count > 0 Then
                ...
                If Me.DBHeader.CompanyID = "LAC" Or Me.DBHeader.CompanyID = "LAH" Then
                    If bIsULinked Then
                        Me.Ctrl_POS_Fund_SWOut.TotalCalmableAmt = dsReceClaim.Tables(0).Rows(0).Item("EstAcctBal")
                        Me.Ctrl_POS_Fund_SWOut.TotalCalmAmt = dsReceClaim.Tables(0).Rows(0).Item("EstWithdrawalAmt")
                        Me.Ctrl_POS_Fund_SWOut.SurCharge = dsReceClaim.Tables(0).Rows(0).Item("Charges")
                        Me.Ctrl_POS_Fund_SWOut.Principle = 0
                        Me.Ctrl_POS_Fund_SWOut.Interest = 0
                        Me.Ctrl_POS_Fund_SWOut.NewSI = dsReceClaim.Tables(0).Rows(0).Item("ReducedSI")
                    ElseIf bIsULife Then
                        Me.Ctrl_POS_Fund_SWOut.TotalCalmableAmt = dsReceClaim.Tables(0).Rows(0).Item("EstAcctBal")
                        Me.Ctrl_POS_Fund_SWOut.TotalCalmAmt = dsReceClaim.Tables(0).Rows(0).Item("EstWithdrawalAmt")
                        Me.Ctrl_POS_Fund_SWOut.SurCharge = dsReceClaim.Tables(0).Rows(0).Item("Charges")
                        Me.Ctrl_POS_Fund_SWOut.Principle = 0
