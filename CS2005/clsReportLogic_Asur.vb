...
Public Class clsReportLogic_Asur
    ...
    Public Sub RPU_Report()
        ...
        sqldt = GetAsurPolicyListCIW("", SingleQuoteFormatted, "PH", "POLST", lngErr, strErr, lngCnt, isGetCountOnly, "ING", "=")
        ...
        If Not sqldtAsur Is Nothing Then
            If sqldtAsur.Rows.Count > 0 Then
                isAssurance = True
                strAsurComp = sqldtAsur.Rows(0)("CompanyID").ToString()
            End If
            'MsgBox("sqldtAsur : " & sqldtAsur.Rows.Count , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            'MsgBox("strAsurComp : " & strAsurComp , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        End If
        ...
        If isBermuda Then
            ...
            'objDBHeader.EnvironmentUse = g_Env '"S106"
            'objDBHeader.ProjectAlias = "LAS" '"LAS"
            'objDBHeader.CompanyID = "ING" '"LAC"
            'objDBHeader.UserType = "LASUPDATE" '"LASUPDATE"

            strConnection = strCIWConn
        ...
        If isAssurance Then
            ...
            'objDBHeader.UserID = gsUser
            'objDBHeader.EnvironmentUse = g_LacEnv '"S106"
            'objDBHeader.ProjectAlias = "LAS" '"LAS"
            'objDBHeader.CompanyID = strAsurComp '"LAC"
            'objDBHeader.UserType = "LASUPDATE" '"LASUPDATE"

            strConnection = strLIDZConn
        End If
        ...
        If GetDT(strSQL, strConnection, dtResult, strErrMsg) Then
            If dtResult.Rows.Count > 0 Then
                If dtResult.Rows(0).Item("CompanyID") = "ING" Or dtResult.Rows(0).Item("CompanyID") = "BMU" Or dtResult.Rows(0).Item("CompanyID") = "LAC" Or dtResult.Rows(0).Item("CompanyID") = "LAH" Then
                    LifeAsia = True
                End If
            End If
        End If
        ...
        If LifeAsia Then
            ...
            If GetDT(strSQL, strConnection, dtResult, strErrMsg) Then
                ...
                If QuoteRpuAsur(strPolicy, False, isAssurance, dsRPU, TotalCSV, strErrMsg) = True Then
    ...
    Public Sub Payment_Rpt()
        ...
        Dim objMQQueHeader As Utility.Utility.MQHeader = New Utility.Utility.MQHeader
        Dim strTime As String = ""
        Dim blnGetPolicy As Boolean
        Dim dr As DataRow
        Dim dtSendData As New DataTable
        Dim objDBHeader As Utility.Utility.ComHeader = New Utility.Utility.ComHeader
        ...
        If isBermuda Then
            ...
            objMQQueHeader.CompanyID = "ING" '"LAC"
            objMQQueHeader.EnvironmentUse = g_Env '"S106"
        ...
        If isAssurance Then
            objMQQueHeader.UserID = gsUser
            objMQQueHeader.QueueManager = g_QmanAsur '"LACSQMGR1" '"WINTEL"
            objMQQueHeader.RemoteQueue = g_WinRemoteAsurQ '"LACSSIT02.TO.LA400SIT02" '"LIFEASIA.RQ1"
            objMQQueHeader.ReplyToQueue = g_LAReplyAsurQ '"LA400SIT02.TO.LACSSIT02" '"WINTEL.RQ1"
            objMQQueHeader.LocalQueue = g_WinLocalAsurQ  '"LACSSIT02.QUEUE1.LCL" '"WINTEL.LQ1"
            objMQQueHeader.Timeout = 90000000 'My.Settings.Timeout

            objMQQueHeader.CompanyID = strAsurComp '"LAC"
            objMQQueHeader.EnvironmentUse = g_LacEnv '"S106"
            objMQQueHeader.ProjectAlias = "LAS"
            objMQQueHeader.UserType = "LASUPDATE"
        ...
        'MsgBox("g_QmanAsur : " & g_QmanAsur , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        'MsgBox("g_WinRemoteAsurQ : " & g_WinRemoteAsurQ , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        'MsgBox("g_LAReplyAsurQ : " & g_LAReplyAsurQ , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        'MsgBox("g_WinLocalAsurQ : " & g_WinLocalAsurQ , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        'MsgBox("g_LacEnv : " & g_LacEnv , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        ...
        If My.Settings.LAReady = True Then
            ...
            'MsgBox("0.0" , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            If dsPolicyCurr.Tables.Count > 0 Then
                If dsPolicyCurr.Tables(0).Rows.Count > 0 Then
                    'MsgBox("0.1" , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
                ...
                End If
            End If
            'MsgBox("0.2" , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        End If

        If IsLARec Then
            'MsgBox("0.3" , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            If dtPoList.Rows.Count > 0 Then
            ...
            'MsgBox("0.4" , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            If dsPolicyCurr.Tables.Count > 0 Then
                If dsPolicyCurr.Tables(0).Rows.Count > 0 Then
                    'MsgBox("0.5" , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        ...
        Else
            'MsgBox("0.6" , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            dtPoList = objCS.GetPolicySummary(strPolicy, lngErrNo, strErrMsg)
            ...
            If dtPoList.Rows.Count > 0 Then
                'MsgBox("0.7" , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
                strAG = Right(dtPoList.Rows(0).Item("POAGCY"), 5)
                For i As Integer = 0 To dtPoList.Rows.Count - 1
                    dtPoList.Rows(i).Item("POAGCY") = strAG
        ...
        End If
        ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''LA rec modify
        'MsgBox("0.8" , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        ...
        Catch ex As Exception
            MsgBox(ex.ToString, MsgBoxStyle.Critical + MsgBoxStyle.OkOnly, "Error")
        End Try
        'MsgBox("0.9" , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        '#If UAT <> 0 Then
    ...
    Public Sub CLAIM_RPT_Bermuda()
        Claim_rpt("ING")
    End Sub

    Public Sub CLAIM_RPT_Asur()
        Claim_rpt("LAC")
    End Sub

    'Claim_rpt() - Prepare data for Claim Breakdown Report
    Public Sub Claim_rpt(Optional ByVal Company As String = "ING")
        Dim strSQL As String
        ...
        Dim cSQL_Prefix As String = cSQL3
        ...
        HandleStringValueByCompany(Company, "", "", "", sqlConn.ConnectionString, cSQL_Prefix)
        ...
        strSQL = "SELECT mcschd_clm_status, mcschd_converted, mcsmsl_message_no " &
                         "FROM " & cSQL_Prefix & ".mcs.dbo.mcs_claim_header_details " &
                         "LEFT OUTER JOIN " & cSQL_Prefix & ".mcs.dbo.mcs_message_log " &
        ...
        strSQL = "SELECT max(mcschd_claim_occur) from " & cSQL_Prefix & ".mcs.dbo.mcs_claim_header_details " &
    ...
    Public Sub PremCallRpt_Bermuda()
        PremCallRpt("ING")
    End Sub

    Public Sub PremCallRpt_Asur()
        PremCallRpt("LAC")
    End Sub


    ' **** ES005 begin ****
    Public Sub PremCallRpt(Optional ByVal Company As String = "ING")
        ...
        If frmParam.ShowDialog() = DialogResult.OK Then
            ...
            strSQL = "Select p.policyaccountid, p.productid, description, p.paidtodate, p.mode, p.aplindicator, p.modalpremium, p.posamt, p.billingtype, cph.nameprefix, cph.namesuffix, cph.firstname, " &
                ...
                " and p.companyid in ('EAA','ING','BMU') " &
                " and p.paidtodate between '" & Format(frmParam.dtP1From.Value, "MM/dd/yyyy") & "' and '" & Format(frmParam.dtP1To.Value, "MM/dd/yyyy") & "' "
            'Sky 20180109  end
            ...
            HandleStringValueByCompany(Company, frmParam.txtPath.Text, "\PremCallRpt_", outputpath, strConn, "")

            If GetDT(strSQL, strConn, dtResult, strError) Then
                If frmParam.CheckBox1.Checked Then
                    If Not ExportCSV(outputpath, dtResult) Then
                    End If
                Else
                    ' All
                    HandleStringValueByCompany(Company, frmParam.txtPath.Text, "\PremCallRpt_all_", outputpath, strConn, "")
                    If Not ExportCSV(outputpath, dtResult) Then
                    End If
                    ' Agency only
                    dtResult.DefaultView.RowFilter = "unitcode >= '00800' and unitcode <= '69999'"
                    HandleStringValueByCompany(Company, frmParam.txtPath.Text, "\PremCallRpt_agency_", outputpath, strConn, "")
                    If Not ExportCSV(outputpath, dtResult) Then
                    End If
        ...
        If frmParam.ShowDialog() = DialogResult.OK Then
            ...
            If frmParam.rdoUL.Checked = True Then
                ...
                strSQL &= "c.nameprefix as PH_Nameprefix, c.namesuffix as PH_Namesuffix, c.firstname as PH_Firstname, c.phonemobile as Tel1, c.phonepager as Tel2, cswpad_add1 as Address1, cswpad_add2 as Address2, cswpad_add3 as Address3, cswpad_city as Address4, " &
                    " c1.nameprefix as SA_Nameprefix, c1.namesuffix as SA_Namesuffix, c1.firstname as SA_Firstname, a.locationcode as Location  , pr.PolicyReplacementInd AS Policy_Replacement, prd.NameOfInsurer AS Internal_Replacement, prd.CompanyName AS Company_Name, prd.policyNumber as old_PolicyNumber " &
                    ...
                    " LEFT JOIN " & gcNBSDB & "application Nbsapplication on p.PolicyAccountID = Nbsapplication.policyNumber " &
                    " LEFT JOIN " & gcNBSDB & "policy_replacement pr on Nbsapplication.Id = pr.applicationId " &
                    " LEFT JOIN " & gcNBSDB & "policy_replacement_detail prd on pr.Id = prd.PolicyReplacementId " &
                    ...
                    " and p.companyid in ('ING','EAA','BMU') "
            Else
            ...
            If GetDT(strSQL, strCIWConn, dtResult, strError) Then
                ...
                If (dtResult IsNot Nothing AndAlso dtResult.Rows.Count <> 0) Then
                    AddAgentCode(dtResult)
                    If ExportCSV(frmParam.txtPath.Text & "\PostSaleCall_" & strType & Format(Today, "MMddyyyy") & ".csv", dtResult) Then
                        MsgBox("Report generated to " & frmParam.txtPath.Text & " sucessfully.", MsgBoxStyle.Information + MsgBoxStyle.OkOnly)
                    Else
                        MsgBox(strError, MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly)
                    End If
                Else
                    MsgBox("No data to export", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly)
                End If
            End If
        ...
        If frmParam.ShowDialog() = DialogResult.OK Then
            ...
            If frmParam.rdoUL.Checked = True Then
                ...
                strSQL &= "c.nameprefix as PH_Nameprefix, c.namesuffix as PH_Namesuffix, c.firstname as PH_Firstname, c.phonemobile as Tel1, c.phonepager as Tel2, cswpad_add1 as Address1, cswpad_add2 as Address2, cswpad_add3 as Address3, cswpad_city as Address4, " &
                    ...
                    " ON p.policyaccountid = cswpuw_poli_id " &
                    " LEFT JOIN csw_policy_address ON cswpad_poli_id = p.policyaccountid " &
                    " LEFT JOIN " & gcNBSDBMcu & "application Nbsapplication on p.PolicyAccountID = Nbsapplication.policyNumber " &
                    " LEFT JOIN " & gcNBSDBMcu & "policy_replacement pr on Nbsapplication.Id = pr.applicationId " &
                    " LEFT JOIN " & gcNBSDBMcu & "policy_replacement_detail prd on pr.Id = prd.PolicyReplacementId " &
                    " where cswpof_Form_Code = 'FNA' " &
                    " and cswpof_Item_name = 'SUITABILITY' " &
                    " and RTRIM(cswpof_Value) in ('A','B','C') " &
                    " and cswpof_policy = p.policyaccountid " &
            ...
            If GetDT(strSQL, strCIWMcuConn, dtResult, strError) Then
                ...
                If (dtResult IsNot Nothing AndAlso dtResult.Rows.Count <> 0) Then
                    AddAgentCode(dtResult)
                    If ExportCSV(frmParam.txtPath.Text & "\PostSaleCall_MCU_" & strType & Format(Today, "MMddyyyy") & ".csv", dtResult) Then
                        MsgBox("Report generated to " & frmParam.txtPath.Text & " sucessfully.", MsgBoxStyle.Information + MsgBoxStyle.OkOnly)
                    Else
                        MsgBox(strError, MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly)
                    End If
    ...
    End Function
    Public Function AddAgentCode(ByRef dtResult As DataTable) As DataTable
        Dim dtAgentCode As DataTable
        Dim strSQL, strError, strActivity As String
        Dim sqlda As SqlDataAdapter
        Dim sqlConn As New SqlConnection
        'Dim serverPrefix As String = ConcatServerDB(gcCRSServer, gcCRSDB)
        Dim policyNoList As New List(Of String)()
        Dim policyNoListString As String = String.Join(",", policyNoList.Select(Function(p) "'" & p & "'"))
        Dim dbConnection As String = strCIWConn

        For Each row As DataRow In dtResult.Rows
            policyNoList.Add(row("Policy_No").ToString())
        Next
        policyNoListString = String.Join(",", policyNoList.Select(Function(p) "'" & p & "'"))

        sqlConn.ConnectionString = strCIWConn
        If (ComFlag = "MC") Then
            sqlConn.ConnectionString = strCIWMcuConn
            dbConnection = strCIWMcuConn
        End If

        strSQL = " Select AgentCode, cust.customerid, PolicyAccountID, GovernmentIDCard, CompanyID from csw_poli_rel rel 
                   LEFT JOIN customer cust on rel.customerid = cust.customerid
                   Where PolicyRelateCode = 'SA' 
                   and PolicyAccountID IN (" & policyNoListString & ")"

        Try
            GetDT(strSQL, dbConnection, dtAgentCode, strError)
        Catch exc As Exception
            MsgBox(exc.ToString, MsgBoxStyle.Critical + MsgBoxStyle.OkOnly, "Get Agent Code failed")
        End Try

        dtResult.Columns.Add("AgentCode", GetType(String))
        Dim locationIndex As Integer = dtResult.Columns("Location").Ordinal
        dtResult.Columns("AgentCode").SetOrdinal(locationIndex + 1)
        Dim agentCodeMap As New Dictionary(Of String, String)()
        For Each row As DataRow In dtAgentCode.Rows
            Dim policyAccountId As String = row("PolicyAccountId").ToString()
            Dim agentCode As String = row("AgentCode").ToString()
            If Not agentCodeMap.ContainsKey(policyAccountId) Then
                agentCodeMap.Add(policyAccountId, agentCode)
            End If
        Next
        For Each row As DataRow In dtResult.Rows
            Dim policyNo As String = row("Policy_No").ToString()
            If agentCodeMap.ContainsKey(policyNo) Then
                row("AgentCode") = agentCodeMap(policyNo)
            Else
                row("AgentCode") = DBNull.Value ' or set to an appropriate default value
            End If
        Next
        Return dtResult
    End Function
    Public Function AddReplacementStatus(ByRef dtResult As DataTable) As DataTable
        dtResult.Columns.Add("CallType")
        Dim sortedDt = dtResult.Clone()
        Dim callType As CRS.CRS.PostSalesCallType
        Dim policyDictionary As New Dictionary(Of String, String)()
        Dim strErr = ""
        Dim getCallType = GetCallTypeForWelcomeCall(dtResult, strErr, ComFlag)
        Try
            For Each row As DataRow In dtResult.Rows
                'WelcomeCall_2ndPhase
                If (String.IsNullOrEmpty(row("Cooling_Off_Date").ToString())) Then
                    Try
                        row("Cooling_Off_Date") = GetCoolingOffDate(row("Policy_No").ToString(), ComFlag)
                    Catch ex As Exception
                    End Try
                End If
                ...
                If row("Policy_Replacement") IsNot DBNull.Value Then
                    ...
                    End Select
                End If
                Dim dtRow As DataRow = sortedDt.NewRow()
                dtRow.ItemArray = row.ItemArray
                If Not ((String.IsNullOrEmpty(row("Tel1").ToString()) AndAlso
                String.IsNullOrEmpty(row("Tel2").ToString()) AndAlso Not (
                (row("Policy_Replacement").ToString() = "Not Determined") AndAlso (
                row("Internal_Replacement").ToString() = "External") AndAlso (
                (row("Company_name").ToString().ToUpper() = "FWD") OrElse (
                row("Company_name").ToString() = "Metlife"))))) Then
                    sortedDt.Rows.Add(dtRow)
                End If
                'End If
            Next
        Catch ex As Exception
    ...
    Public Function ACC_REPORT_Bermuda()
        ACC_REPORT("ING")
    End Function

    Public Function ACC_REPORT_Asur()
        ACC_REPORT("LAC")
    End Function

    Public Function ACC_REPORT(Optional ByVal Company As String = "ING")
        ...
        Dim outputpath As String = String.Empty
        HandleStringValueByCompany(Company, frmParam.txtPath.Text, "\ACCRpt_", outputpath, strConn, "")
    ...
    Public Sub FCR_REPORT_Bermuda()
        FCR_Report("ING")
    End Sub

    Public Sub FCR_REPORT_Asur()
        FCR_Report("LAC")
    End Sub

    Public Sub FCR_Report(Optional ByVal Company As String = "ING")
        ...
        If frmParam.ShowDialog() = DialogResult.OK Then
            ...
            Dim outputpath As String = String.Empty
            HandleStringValueByCompany(Company, frmParam.txtPath.Text, "\FCRRpt_", outputpath, strConn, "")
            If GetDT(strSQL, strConn, dtResult, strError) Then
                If Not ExportCSV(outputpath, dtResult) Then
                End If
    ...
    Public Function LA_PolicyDetail()
        ...
        If frmParam.ShowDialog() = DialogResult.OK Then
            strPolicy = frmParam.PolicyNo.Trim
            frmParam.Dispose()
        Else
            frmParam.Dispose()
            Exit Function
        ...
        'If Not policySearchBL.SearchPolicy(strPolicy, policySearchErr, policyDs, policyDt) = True
        '    MsgBox(policySearchErr)
        'End If
        ...
        If isBermuda Then
            Me.CompanyID = sqldt.Rows(0)("CompanyID")
        End If

        If isAssurance Then
            Me.CompanyID = sqldtAsur.Rows(0)("CompanyID")
        End If
        'MsgBox("CompanyID : " & Me.CompanyID , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")

        sqlConn.ConnectionString = GetConnectionStringByCompanyID(Me.CompanyID)
        strCIWConn1 = GetConnectionStringByCompanyID(Me.CompanyID)
        'sqlConn.ConnectionString = strCIWConn1
        Dim serverPrefix As String = ConcatServerDB(gcCRSServer, gcCRSDB)
        If Me.CompanyID = "LAC" OrElse Me.CompanyID = "LAH" Then
            serverPrefix = ConcatServerDB(gcCRSServer, gcCRSDBAsur)
        ElseIf Me.CompanyID = "MCU" Then
        ...
        Dim dtCoRider As DataTable = New DataTable
        connection.Open()
        'MsgBox("1.01", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        ...
        Try
            ...
            adapter.SelectCommand.CommandTimeout = connection.ConnectionTimeout
            adapter.Fill(dtCoRider)
            'MsgBox("1.02", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        Catch ex As Exception
            Console.WriteLine(ex.Message)
            Throw
        Finally
            connection.Close()
        End Try
        'MsgBox("1.03", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        Try
            sqlda = New SqlDataAdapter(strSQL, sqlConn)
            sqlda.TableMappings.Add("DDA1", "CCDR")
            sqlda.TableMappings.Add("DDA2", "BillingTypeCodes")
            ...
            sqlda.TableMappings.Add("DDA14", "Product_Chi")
            sqlda.Fill(dsMisc, "DDA")
            'MsgBox("1.04", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        Catch sqlex As SqlClient.SqlException
            MsgBox(sqlex.Number & sqlex.ToString, MsgBoxStyle.Critical + MsgBoxStyle.OkOnly, "Error")
        Catch ex As Exception
            MsgBox(ex.ToString, MsgBoxStyle.Critical + MsgBoxStyle.OkOnly, "Error")
        End Try
        'MsgBox("1.05", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        If dsMisc.Tables("PolicyAccount").Rows.Count = 0 Then
            MsgBox("Policy not found - " & strPolicy, MsgBoxStyle.Exclamation, "Policy Letter")
            Exit Function
        End If
        'MsgBox("1.06", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        If dsMisc.Tables("product_type").Rows.Count > 0 Then
            If IsDBNull(dsMisc.Tables("product_type").Rows(0).Item("PrintValueReport")) Then
                MsgBox("Policy Letter is not available for this product yet." & strPolicy, MsgBoxStyle.Exclamation, "Policy Letter")
                Exit Function
            ElseIf dsMisc.Tables("product_type").Rows(0).Item("PrintValueReport") = "A" Then
                'MsgBox("1.07", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
                For Each dr As DataRow In dtPolicyInfo.Rows
                    If Mid(dr.Item("ProductID"), 2, 4) = "RE15" OrElse Mid(dr.Item("ProductID"), 2, 4) = "RE20" Then
                        MsgBox("Policy Letter is not available, HRE15/HRE20/URE15/URE20 is attached." & strPolicy, MsgBoxStyle.Exclamation, "Policy Letter")
                        Exit Function
            ...
            End If
        End If
        'MsgBox("1.08", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        If dsMisc.Tables("PolicyAccount").Rows(0).Item("CompanyID") = "EAA" Then
            'MsgBox("1.09", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            Dim objDB As Object
            ...
            If objRS.RecordCount > 0 Then
                'MsgBox("1.10", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            ...
            End If
            'MsgBox("1.11", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        Else
            'MsgBox("1.12", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            ' Call Web service to get value
            Dim dtVAL As DataTable
            Dim dblTotal As Double
            'MsgBox("CompanyID : " & CompanyID, MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            'MsgBox("ProductPolValueFunc : " & dsMisc.Tables("product_type").Rows(0).Item("ProductPolValueFunc"), MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            dtVAL = GetLAPolicyValue(CompanyID, strPolicy, dsMisc.Tables("product_type").Rows(0).Item("ProductPolValueFunc"), dblTotal, strErr)
            'MsgBox("After GetLAPolicyValue", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            'MsgBox("strErr : " & strErr, MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            If strErr = "" Then
                If dtVAL IsNot Nothing AndAlso dtVAL.Rows.Count > 0 Then
                    'MsgBox("1.13", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
                    With dtVAL
                        Dim drVal As DataRow = dsMisc.Tables("csw_policy_value").NewRow
                        drVal("cswval_id") = -1
                    ...
                    End With
                Else
                    'MsgBox("1.14", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
                    With dtVAL
                        Dim drVal As DataRow = dsMisc.Tables("csw_policy_value").NewRow
                        drVal("cswval_id") = -1
                        drVal("cswval_TFLOID") = "Y"
        ...
        'Next
        Try
            'MsgBox("1.15", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            rpt.Database.Tables("PolicyAccount").SetDataSource(dsMisc.Tables("PolicyAccount"))
            rpt.Database.Tables("csw_ing_logo_table").SetDataSource(dsMisc.Tables("csw_ing_logo_table"))
            ...
            Try
                'MsgBox("1.16", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
            ...
            Try
                'MsgBox("1.17", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
...
#Region "Post-sales call list"
    ...
    Public Sub PostSalesCallList(ByVal printFromDate As DateTime, ByVal printToDate As DateTime, ByVal strFileName As String, Optional ByVal sCompID As String = "HK")
        Dim strSQL As String = String.Empty
        Dim strErr As String = String.Empty
        ...
        Using cnn As New SqlConnection(strConn), cmd As New SqlCommand("cswsp_GetPostSalesCallList", cnn)
            ...
            If "BMU".Equals(sCompID) Then
                cmd.Parameters.Add("@companyID", SqlDbType.VarChar, 10).Value = sCompID
            End If
        ...
        If Not "MC".Equals(sCompID) Then
            For Each dr As DataRow In dtCallList.Rows
                If dr("CompanyID").ToString().Trim() = "ING" OrElse dr("CompanyID").ToString().Trim() = "BMU" Then
                    Try
...
#Region "APL Loan Letter"
    ...
    Public Sub AplLoanLetter()
        ...
        Dim intCnt As Integer
        Dim datFrom, datStart, datEnd, datLast As Date
        Dim dtLoan, dtPoList, dtPolMisc As New DataTable
        'updated at 2023-10-17 by oliver 
        Dim dtLoan_All As DataTable
        Dim blnCont As Boolean = True
        ...
        strPolicy = frmInput.PolicyNo
        datFrom = frmInput.FromDate
        datStart = datFrom
        datEnd = frmInput.ToDate
        ...
        Dim strConnection As String = String.Empty
        Dim strAsurComp As String = String.Empty
        'oliver 2024-1-30 added for update AplLoanLetter Crystal Reports footer
        ...
        If Not sqldt Is Nothing Then
            If sqldt.Rows.Count > 0 Then
                ...
                isBermuda = True
                'oliver 2024-1-30 added for update AplLoanLetter Crystal Reports footer
                strCompanyCode = sqldt.Rows(0)("CompanyID").ToString()
            End If
            'MsgBox("sqldt : " & sqldt.Rows.Count , MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Checking")
        End If
        ...
        If My.Settings.LAReady = True Then
            ...
            'MsgBox("Test Check err : " & strErr)

            blnGetPolicy = clsPOS.GetPolicy(dsPolicySend, dsPolicyCurr, strTime, strErr)
            'MsgBox("Check GetPolicy : " & IIf(blnGetPolicy, "1", "0"))
            'MsgBox("Check dsPolicyCurr table count : " & dsPolicyCurr.Tables.Count.ToString)
            If dsPolicyCurr.Tables.Count > 0 Then
                If dsPolicyCurr.Tables(0).Rows.Count > 0 Then
        ...
        If dsPolicySend.Tables.Count > 0 Then
            dsPolicySend.Tables.RemoveAt(0)
        End If
        If dsPolicyCurr.Tables.Count > 0 Then
            dsPolicyCurr.Tables.RemoveAt(0)
        ...
        'MsgBox("Check Params RTrim(strPolicy) : " & RTrim(strPolicy) & " , datEnd : " & datEnd.ToString & " , datFrom : " & datFrom.ToString )
        'Call LifeAsia BO APLTRN to get loan and APL history
        blnGetPolicy = clsPOS.GetLoanAndAPLHistory(RTrim(strPolicy), datFrom, datEnd, dsPolicyCurr, strErr)

        'MsgBox("Check GetLoanAndAPLHistory : " & IIf(blnGetPolicy, "1", "0"))
        'MsgBox("Check dsPolicyCurr table count : " & dsPolicyCurr.Tables.Count.ToString)
        If dsPolicyCurr.Tables.Count > 0 Then
            If dsPolicyCurr.Tables(0).Rows.Count > 0 Then
                'MsgBox("Check dsPolicyCurr table(0) row count : " & dsPolicyCurr.Tables(0).Rows.Count.ToString)
                ...
                'MsgBox("Check dtLoan rows count : " & dtLoan.Rows.Count.ToString)
                lngErrNo = 0
                ...
                If lngErrNo = 0 Then
                    ...
                    For i = 0 To intCnt
                        'updated at 2023-10-17 by oliver 
                        If dtLoan.Rows(i).Item("TransDate") <> datLast And dtLoan.Rows(i).Item("TransDate") >= datFrom Then
        ...
        End If
        ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''LA rec modify
        'MsgBox("Check dtLoan_All rows count : " & dtLoan_All.Rows.Count.ToString)
        ...
        Dim cswiltLogoDesc As String = "ING Logo BW"

        If isAssurance Then
            cswiltLogoDesc = "ING Logo BW ASUR"
        End If

        strSQL = "SELECT '" & strPolicy & "' as Policy, ING_Logo = a.cswilt_Logo, Address = b.cswilt_Logo, IService = c.cswilt_Logo, ICaring = d.cswilt_Logo " &
               "FROM (select * from " & gcPOS & "vw_csw_ing_logo_table where cswilt_LogoDesc = '" & cswiltLogoDesc & "') a " &
               ", (select * from " & gcPOS & "vw_csw_ing_logo_table where cswilt_LogoDesc = 'Footer ING Address') b " &
               ", (select * from " & gcPOS & "vw_csw_ing_logo_table where cswilt_LogoDesc = 'Footer ING Interactive Service') c " &
               ", (select * from " & gcPOS & "vw_csw_ing_logo_table where cswilt_LogoDesc = 'Footer Caring Company') d "
        ...
        If GetDT(strSQL, strConnection, dtSQLResult, strErrMsg) Then
        ...
        Try
            ...
            'oliver 2024-1-30 added for update AplLoanLetter Crystal Reports footer
            rpt.SetParameterValue("strCompanyCode", strCompanyCode)
        Catch ex As Exception
            MsgBox(ex.ToString, MsgBoxStyle.Critical + MsgBoxStyle.OkOnly, "Error")
        End Try
...
#End Region
    Public Function HandleStringValueByCompany(ByVal Company As String, ByVal txtPath As String, ByVal fileName As String, ByRef outputpath As String, ByRef strConn As String, ByRef cSQL_Prefix As String)

        If Company = "ING" Then
            strConn = strCIWConn
            If Not String.IsNullOrEmpty(txtPath) AndAlso Not String.IsNullOrEmpty(fileName) Then
                outputpath = txtPath & fileName & Format(Today, "MMddyyyy") & ".csv"
            End If
            If Not String.IsNullOrEmpty(cSQL_Prefix) Then
                cSQL_Prefix = cSQL3
            End If
        ElseIf Company = "LAC" Or Company = "LAH" Then
            strConn = strLIDZConn
            If Not String.IsNullOrEmpty(txtPath) AndAlso Not String.IsNullOrEmpty(fileName) Then
                outputpath = txtPath & fileName & Format(Today, "MMddyyyy") & "_Asur" & ".csv"
            End If
            If Not String.IsNullOrEmpty(cSQL_Prefix) Then
                cSQL_Prefix = cSQL3_Asur
            End If
        End If
...
#Region "001"
    ...
    Private Function GetConnectionStringForPrintLetter(ByVal policyNo As String, ByRef strCompanyID As String, Optional ByRef isHnwPolicy As Boolean = False) As String
        ...
        If isBermuda Then
            If Not IsDBNull(sqldt.Rows(0)("CompanyID")) AndAlso Not String.IsNullOrEmpty(sqldt.Rows(0)("CompanyID").ToString) Then
                ...
                If sqldt.Rows(0)("CompanyID").ToString = "BMU" Then
    ...
    Public Sub PrintLetter(ByVal letterType As String)
        ...
        'oliver 2024-8-6 added for Com 6
        Dim isHnwPolicy As Boolean = False
        Dim strCIWConn As String = GetConnectionStringForPrintLetter(policyNo, strCompanyID, isHnwPolicy)
        If isHnwPolicy AndAlso Not CheckUserRight("CRS_HNW_USER") Then
            MsgBox($"{gsUser} has no Private permission , You’re not authorized to access.")
            Exit Sub
        End If
        ...
        Select Case letterType
            Case "PSC follow letter"
                ...
                dtField = GetPSCFollowLetterData(policyNo, dsRegion, strCIWConn, strCompanyID)
                ltrCode = "PSCFollowHK"
            Case "Payment letter"
                ...
                ltrCode = If(frmInput.radChi.Checked, If(isHnwPolicy, "PaymentChiBMU", "PaymentChi"), If(isHnwPolicy, "PaymentEngBMU", "PaymentEng"))
            Case "Policy letter"
                'oliver 2024-3-13 updated for NUT-3813
                dtField = GetPolicyLetterData(policyNo, dsRegion, frmInput.radChi.Checked, strCIWConn)
                'oliver 2024-8-6 updated for Com 6
                ltrCode = If(frmInput.radChi.Checked, If(isHnwPolicy, "PolicyChiBMU", "PolicyChi"), If(isHnwPolicy, "PolicyEngBMU", "PolicyEng"))
        End Select
    ...
    'oliver 2024-3-13 updated for NUT-3813
    Private Function GetPSCFollowLetterData(ByVal policyNo As String, ByRef dsRegion As DataSet, ByVal strCIWConn As String, ByVal strCompanyID As String) As DataTable
        ...
        Dim dtContractDtl As DataTable = GetContractDetail(policyNo, strCompanyID)
        ...
        'get event detail -s
        'oliver added 2024-6-27 for Assurance Production Version hot fix
        Dim dtServiceLog As DataTable = GetServiceLogInfo(policyNo, strCIWConn, strCompanyID)
        ...
        'LAS logic s
        Dim dtCoverage As DataTable = GetCOSelWithCustNo(policyNo, strCompanyID)
        ...
        Dim dtPremiumRoutine As DataTable = GetPremiumRoutine(policyNo, paidToDate, strFreq, strCompanyID)
        ...
        For Each dr As DataRow In dtCoverage.Rows
            ...
            If dtPremiumRoutine.DefaultView.Count > 0 Then
                ...
                If booster <> 0.00 Then
                    ...
                    'oliver added 2024-6-27 for Assurance Production Version hot fix
                    Dim dtPSCPS As DataTable = GetPostSalesCallProductSetting(productId, strCIWConn, strCompanyID)
    ...
    Private Function GetPolicyLetterValue(ByVal companyId As String, ByVal policyNo As String, ByVal additionDeathCvr As String, ByVal productPolValueFunc As String, ByVal strCIWConn As String) As DataTable
        ...
        Else
            ...
            Dim dtVAL As DataTable = GetLAPolicyValue(companyId, policyNo, productPolValueFunc, dblTotal, strErr)
            If strErr = "" Then
                If dtVAL IsNot Nothing AndAlso dtVAL.Rows.Count > 0 Then
                    With dtVAL
                        Dim dr As DataRow = dtPolicyValue.NewRow
