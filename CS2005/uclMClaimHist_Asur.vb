...
#End Region
    ...
    Private Sub InitUI()
        ...
        If objMQQueHeader.CompanyID = "LAC" Or objMQQueHeader.CompanyID = "LAH" Then
    ...
    Private Sub RefreshData()
        ...
        Dim POSDB As String
        Dim LCPDB As String
        If gUAT = False OrElse strCompany = "LAC" OrElse strCompany = "LAH" Then
            CIWDB = Utility.Utility.GetDbNameByCompanyEnv_CIW(strCompany, g_LacEnv) & ".."
            MCSDB = Utility.Utility.GetDbNameByCompanyEnv_MCS(strCompany, g_LacEnv) & ".."
            POSDB = Utility.Utility.GetDbNameByCompanyEnv_POS(strCompany, g_LacEnv) & ".."
            LCPDB = GetLcpDbName(strCompany, g_LacEnv) & ".."
        Else
            CIWDB = gcCIW
            MCSDB = gcMCS
            POSDB = gcPOS
        ...
        sqlConnect = New SqlConnection(GetConnectionStringByCompanyID(strCompany))
        sqlUpdConnect = New SqlConnection(strUPSConn)

        'ITSR933 FG R3 CE Start
        'Dim strPolicyCap As String = GetCapsilPolicyNo(m_PolicyNumber)
        ...
        strSQL = "Select *, CompanyID" &
            ...
            " where mjcply_policy_no in ('" & m_PolicyNumber & "') " &
            " And mjcchd_clm_status <> 'DEL' " &
            " Order by mjcply_claim_no DESC, mjcply_claim_occur DESC;"

        If objMQQueHeader.CompanyID = "LAC" Or objMQQueHeader.CompanyID = "LAH" Then
            strSQL += " SELECT a.mjcpay_policy_no, a.mjcpay_claim_no, a.mjcpay_claim_occur, a.mjcpay_cheque_curr, a.mjcpay_cheque_amount,  " &
                    " c.lcpchq_cheque_no as [mjcpay_cheque_no], c.lcpchq_issue_date as [mjcpay_cheque_issue], b.lcpcpm_payee_name as [mjcpay_payee] " &
                    " FROM " & MCSDB & "mjc_claim_payment a left join " & LCPDB & "lcp_cheque_payment b " &
                    " on a.mjcpay_policy_no = b.lcpcpm_policy_no and a.mjcpay_claim_no = b.lcpcpm_case_no " &
                    " left join " & LCPDB & "lcp_cheque c on b.lcpcpm_payment_no = c.lcpchq_payment_no " &
                    " Where a.mjcpay_policy_no in ('" & m_PolicyNumber & "') And isnull(a.mjcpay_policy_no, '') <> '' And isnull(a.mjcpay_payee, '') <> '';"
        Else
            strSQL += " Select * " &
                    " From " & MCSDB & "mjc_claim_payment " &
                    " Where mjcpay_policy_no in ('" & m_PolicyNumber & "') And isnull(mjcpay_policy_no,'') <> '';"
        End If
        ...
        Try
            ...
            For Each drPol As DataRow In dsClaim.Tables("mjc_claim_policy").Rows
                ...
                If drPol("CompanyID").ToString().Trim() = "ING" Or drPol("CompanyID").ToString().Trim() = "BMU" Then
        ...
        " where mjccbd_policy_no in ('" & m_PolicyNumber & "') And isnull(mjccbd_policy_no,'') <> ''"
        ...
        If Not (tempdt Is Nothing) Then
            If tempdt.Rows.Count > 0 Then
                For Each dr In tempdt.Rows
                    'SQL2008
                    sSQL = "UPDATE mjc_claim_register " &
                           "SET mjcchd_comment = N'" & Replace(dr("mjcchd_comment"), "'", "''") & "' " &
                           "WHERE mjcchd_claim_no = " & CType(dr("mjcchd_claim_no"), String) & " AND mjcchd_claim_occur = " & CType(dr("mjcchd_claim_occur"), String)

                    cmd.CommandText = sSQL

                    Try
                        cmd.Connection = sqlUpdConnect
                        sqlUpdConnect.Open()
                        cmd.ExecuteNonQuery()
                        sqlUpdConnect.Close()
...
#End If
                ...
                response = wsCRS.ClaimsAudit(g_Comp, g_Env, ClaimNo, PolicyNo, InsuredName, OccurNo, EventName)
    ...
    Private Sub btnIwsClaimHist_Click(sender As Object, e As EventArgs) Handles btnIwsClaimHist.Click
        ...
        iwsClaimHist.LoadHist(strCompany, m_PolicyNumber)
