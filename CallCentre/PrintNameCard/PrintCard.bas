...
'                    End If
                ...
                'Add WC102007    Put Policy=para1,  Insured name=para2,   company=para4
                    If UCase(Trim(PstrCardType)) = UCase(Trim("Medical SOS Card")) Or _
                       ...
                       para1 = rs.Fields("PolicyNo") & "        " & rs.Fields("Employee") & "-" & rs.Fields("RelCde") ' Policy No  Emp-EE
...
NextCard:
            ...
            'ITDTLW005 Start ***********************************************************************
            Case "MCU"
                'ITDWWI rewrite to use ADODB for UniCode handling
                Dim sExportMCU As String
                sExportMCU = App.Path & "\" & Pstrdb & "PINLetter.txt"
                Dim objStreamMCU As Object
                Set objStreamMCU = CreateObject("ADODB.Stream")
                objStreamMCU.Charset = "UTF-8"
                objStreamMCU.Open
                
                Dim sHeader As String
                sHeader = "!$PINLetter" & vbNewLine
                sHeader = sHeader & "%%PROG: C:\ctrsys\" & Pstrdb & "PINLetter.JDT" & vbNewLine
                objStreamMCU.WriteText sHeader

                
                Dim sExportHK As String
                sExportHK = App.Path & "\" & Pstrdb & "PINLetterHK.txt"
                Dim objStreamHK As Object
                Set objStreamHK = CreateObject("ADODB.Stream")
                objStreamHK.Charset = "UTF-8"
                objStreamHK.Open
                objStreamHK.WriteText sHeader
                    ...
                    Else
                        If Trim(rs2.Fields(4).Value) = "N" Then
                            ...
                            If GetPassword(rs.Fields(0).Value, strID, Pstrdb) = False Then
                                GoTo errhandler
                            End If
                            'ITDWWI Rewrite to use ADODB.Stream for UTF-8
                            Dim sMessage As String
                            sMessage = "Letter|" & Format(Date, "dd mmm yyyy") & "|" & Trim(rs1.Fields(0).Value) & "|" & strID & "|"
                            sMessage = sMessage & Trim(rs2.Fields(0).Value) & "|" & Trim(rs2.Fields(1).Value) & "|" & Trim(rs2.Fields(2).Value) & "|" & Trim(rs2.Fields(3).Value) & "|"
                            sMessage = sMessage & vbNewLine
                            If Trim(rs2.Fields(5).Value) <> "HK" Then
                                objStreamMCU.WriteText sMessage
                            Else
                                objStreamHK.WriteText sMessage
                            End If
                            If Upd_DB_Letter(PstrCardType, rs.Fields(0).Value) = False Then
                                strsql = "INSERT " & PstrCardType & "_print_errlog values (" & rs.Fields(0).Value & ",getdate(),'Cannot Update Print Letter Database for this user!!')"
                                Call db.ExecuteStatement(strsql)
...
NextCardM:
                ...
                If Dir(sExportMCU) <> "" Then
                    Kill sExportMCU
                End If
                
                If Dir(sExportHK) <> "" Then
                    Kill sExportHK
                End If
                sFooter = "%%EOJ" & vbNewLine
                objStreamMCU.WriteText sFooter
                objStreamMCU.SaveToFile sExportMCU
                objStreamMCU.Close
                
                objStreamHK.WriteText sFooter
                objStreamHK.SaveToFile sExportHK
                objStreamHK.Close
                Set objStreamMCU = Nothing
                Set objStreamHK = Nothing
