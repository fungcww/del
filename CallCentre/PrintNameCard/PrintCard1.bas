...
AddOne = False
End Function
Public Sub Print_Label_Job(rsCust As ADODB.Recordset)
Dim Name As String
    ...
    If GetPrinter(prt, LABEL_PRINTER_NAME) Then
        ...
        If result = vbYes Then
            While Not rsCust.EOF
                ...
                Name = GetName(CustId)
                If (Print_Label(CustId, Name, row, col) = True) Then
...
err:
End Function
Public Function GetAddress(ByVal CustId As String) As ADODB.Recordset
Dim strsql As String
...
End Function
Public Function GetPassword(ByVal CustId As String, ByRef PWD As String) As Boolean
On Error GoTo err:
Dim strsql As String
PWD = ""
...
Dim strMemNo As String
    ...
    If Get_Connect(Pstrdb) Then
        Set rs = Get_Print_Letter_Job_Data(PstrCardType, Pstrdb)
        Select Case Pstrdb
            Case "Life"
                ...
                While Not rs.EOF
                            ...
                            If objDBS.Connect(s400User, s400Proj, s400Conn, s400Type) Then
                                ...
                                If Not objRS Is Nothing Then
                                    If objRS.RecordCount > 0 Then
                                        ...
                                        If IsNull(objRS("PORGDT")) Then GoTo NextCard
                                        If Len(objRS("PORGDT")) <> 7 Then GoTo NextCard
                    ...
                    Set rs2 = GetAddress(rs.Fields(0).Value)
                    ...
                    Else
                        If Trim(rs2.Fields(4).Value) = "N" Then
                            ...
                            End If
                            If GetPassword(rs.Fields(0).Value, strID) = False Then
                                GoTo errhandler
                            End If
...
'                  " cswpcr_prtedletter='N' order by convert(varchar(12),cswpcr_crtdate,103),cswpcr_pid"
         strsql = "select cswpcr_cid, cswpcr_prtidxletter, cswpcr_pid, cswpcr_crtdate " & _
                  "from csw_print_cardletter_report " & _
                  "where ((DATEDIFF(day, cswpcr_crtdate, getdate()) >= " & 1 & " and cswpcr_prtidxletter = 'P')" & _
                  " or (cswpcr_prtidxletter = 'R' and cswpcr_prtidxcard <> 'R' and DATEDIFF(day, cswpcr_crtdate, getdate()) >= 1) " & _
                  " or (cswpcr_prtidxletter = 'R' and cswpcr_prtidxcard = 'R' and DATEDIFF(day, cswpcr_crtdate, getdate()) >= 5)) and " & _
                  " cswpcr_prtedletter='N' order by convert(varchar(12),cswpcr_crtdate,103),cswpcr_pid"
' Check Freelook date end
...
Public Sub getPrintOut(pstrMM As String, pstrYY As String)
Dim str1 As String
